% function []=LogNFieldApprox2D_ScriptDemo2()
...
%
% Log: 
% ScriptDemo2: create from 1. try to comform the coarse and fine grid with
%              exactly same eigenvectors, including directions.
%
% Author:   Wei Xing
% History:  12/09/2017  file created
...
coarseKlBasis2=coarseKlBasis*sqrt(coarseklEigenValue);
...
%     surface=scatteredInterpolant(coarseLocation,klBasis(:,i),'nearest');
    ...
    surface=scatteredInterpolant(coarseLocation,coarseKlBasis2(:,i));
    interpolatedKlBasis2(:,i)=surface(location);
end


% interpolatedMuX=log(muY)-diag(fineKlBasis*klEigenValue*fineKlBasis')./2; %not feasible for high resolution
interpolatedMuX=repmat(muX(1,1),size(interpolatedKlBasis,1),1);    %this is wrong solution. But zero mu is assumed here thus OK.
% 
...
showIndex=3:4;
figure(1)
basis=abs(klBasisX*sqrt(klEigenValueX));
plot(basis(:,showIndex),'-')
hold on 
basis=abs(interpolatedKlBasis*sqrt(coarseklEigenValue));
plot(basis(:,showIndex),'--')
hold off 
...
showIndex=1:2;
figure(2)
basis=klBasisX*sqrt(klEigenValueX);
plot(basis(:,showIndex),'-')
hold on 
plot(coarseKlBasis2(:,showIndex),'+')
plot(interpolatedKlBasis2(:,showIndex),'--')
hold off 
title(sprintf('Eigen function compare'))
