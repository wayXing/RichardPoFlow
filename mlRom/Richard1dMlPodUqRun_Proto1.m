...
for i=1:nCluster
    ...
    scatter(xTrain(index,1)',xTrain(index,2)',25,i*50*ones(length(index),1),'filled')
end
...
switch iMethod
    ...
    case 2 
        model = fitcecoc(xTrain,labelTrian);
        labelTest = predict(model,xTest);
    case 3
        %convert class label
        labelTrianU=repmat(labelTrian,1,nCluster)== repmat(1:nCluster,size(xTrain,1),1);

        net = trainSoftmaxLayer(xTrain',labelTrianU');
        labelTestU = net(xTest');
        
%         net = patternnet(10);
%         [net,tr] = train(net,xTrain',labelTrian');

        
        labelTest =sum(labelTestU'.* repmat(1:nCluster,size(xTest,1),1),2);
        labelTest=round(labelTest);
end
...
%     end
    ...
    for j=1:length(index)
        for t=1:nTime
            kRecord(:,t,j)=K(HTrain(:,t,index(j)),KsTrain(index(j),:)'); %TODO WRONG
        end  
    end
